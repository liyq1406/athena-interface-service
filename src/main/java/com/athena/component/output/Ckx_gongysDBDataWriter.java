 ...
 *
 */
public class Ckx_gongysDBDataWriter extends DBOutputTxtSerivce{
	public Ckx_gongysDBDataWriter(DataParserConfig dataParserConfig) {
	...
	protected void outPut(ExchangerConfig write, ExchangerConfig read) throws SQLException{
		int totalPage = getTotalPage(write.getUsercenter(), read.getSql(),read.getIsAllSet());
	  	try {
	    	...
	    	/*********************************************************/
	    	TableElement.addAttribute("name", "供应商参考系(输出)");
	        ...
	        for(int i=0;i<totalPage;i++){
	        	...
	        	rowElement.addAttribute("GONGYSDM", dataList.get(j).get("GCBH").toString());
	        	rowElement.addAttribute("GONGYSMC", strNull(dataList.get(j).get("GONGSMC")));  	
				}
	        }
