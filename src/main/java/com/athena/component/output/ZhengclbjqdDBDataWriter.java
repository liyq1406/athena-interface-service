...
public class ZhengclbjqdDBDataWriter extends DBOutputTxtSerivce {

	public ZhengclbjqdDBDataWriter(DataParserConfig dataParserConfig) {
	...
	@Override
	public void before() {
		logger.info("线程--接口" + interfaceId +" 开始清空整车零部件清单中间表In_Zhengclbjqd，并将查询的整车零部件清单数据写到中间表");	
		dataParserConfig.getBaseDao().getSdcDataSource(sourceId).execute("outPut.truncateInZhengclbjqd");
		int num = dataParserConfig.getBaseDao().getSdcDataSource(sourceId).execute("outPut.insertInZhengclbjqd");
		logger.info("线程--接口" + interfaceId +" 将整车零部件清单数据写到中间表In_Zhengclbjqd结束，插入"+num+"条");
	  	...
	  	try {
	        ...
	        for(int i=0;i<totalPage;i++){
	          ...
	          //XML文件AppReqBody之前数据
		    	...
		    	/*********************************************************/
		    	TableElement.addAttribute("name", "整车零部件清单");
	            ...
	            //row
		        	...
		        	rowElement.addAttribute("ZCBM", dataList.get(j).get("ZCBM").toString().trim());
		        	rowElement.addAttribute("ZCBB", dataList.get(j).get("ZCBB").toString().trim());
		        	rowElement.addAttribute("ZCZT", dataList.get(j).get("ZCZT").toString().trim());
		        	rowElement.addAttribute("POSNR", dataList.get(j).get("POSNR").toString().trim());
		        	rowElement.addAttribute("LJH", dataList.get(j).get("LJH").toString().trim());
		        	rowElement.addAttribute("AENNR", dataList.get(j).get("AENNR").toString().trim());
		        	rowElement.addAttribute("MPDSC", dataList.get(j).get("MPDSC").toString().trim());
		        	rowElement.addAttribute("ZPLX", dataList.get(j).get("ZPLX").toString().trim());
		        	rowElement.addAttribute("GWBM", dataList.get(j).get("GWBM").toString().trim());
		        	rowElement.addAttribute("PVXHDBM", dataList.get(j).get("PVXHDBM").toString().trim());
		        	rowElement.addAttribute("XHDBM", dataList.get(j).get("XHDBM").toString().trim());
		        	rowElement.addAttribute("DCSL", dataList.get(j).get("DCSL").toString().trim());
		        	rowElement.addAttribute("FSRQ", dataList.get(j).get("FSRQ").toString().trim());
		        	rowElement.addAttribute("FSSJ", dataList.get(j).get("FSSJ").toString().trim());
		        	rowElement.addAttribute("LJJM", dataList.get(j).get("LJJM").toString().trim());
		        	rowElement.addAttribute("LRGLFS", dataList.get(j).get("LRGLFS").toString().trim());
		        	rowElement.addAttribute("BDWZ", dataList.get(j).get("BDWZ").toString().trim());
		        	rowElement.addAttribute("SFJS", dataList.get(j).get("SFJS").toString().trim());
				}
	            time ++ ;
	            /*********************************************************/
	            try {
	...
	private int getPageSize(){
		InputStream  in = BOMchuangjDBDataWriter.class.getResourceAsStream("/config/exchange/urlPath.properties");
		Properties pp = new Properties();
		int pageSize = 0;
		try {
			pp.load(in);
			pageSize = Integer.parseInt(pp.getProperty("pageSizeLBJ"));
		} catch (IOException e) {
