 ...
 */
@Component
public class LingjckHZDataWriter extends DuoJieKouOutputWriter {
	public LingjckHZDataWriter() {
			
	}
	
	/**
	 * 2790输出,先将biaos状态null改为1，将1的汇总，同时将1修改为2，然后将数据移动到_H表,最后将2的删除。
	 * @throws SQLException 
	 */
	...
	public void beforeExecute(AbstractIBatisDao baseDao,String sourceId,String id){
		...
		//2汇总查询要货令表达交付表中的数据，根据用户中心，零件编号，物理点编号汇总
		List<HashMap<String, Object>> yaohldbjf = baseDao.getSdcDataSource(sourceId).select("outPut.lingjckHzQuery",param);
		...
		if(yaohldbjf !=null && yaohldbjf.size()>0){
			...
			for(HashMap<String, Object> yaohldbjfMap : yaohldbjf){
				try {
						...
						sqlbuf.append("UPDATE "+SpaceFinal.spacename_zxc+".ckx_lingjck L SET  L.edit_time = to_date('"+nowTime+"','yyyy-mm-dd hh24:mi:ss')");
						sqlbuf.append(",L.DINGDBDZL = NVL(L.DINGDBDZL,0)+"+yaohldbjfMap.get("DINGDBDZL"));
						sqlbuf.append(",L.DINGDZZZL = NVL(L.DINGDZZZL,0)+"+yaohldbjfMap.get("DINGDZZZL"));
						sqlbuf.append(",L.YIJFZL = NVL(L.YIJFZL,0)+"+yaohldbjfMap.get("YIJFZL"));
						...
						sqlbuf.append("' and L.CANGKBH = '"+strNull(yaohldbjfMap.get("WULDBH"))+"'");
