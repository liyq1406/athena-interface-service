...
public class BOMgenggDBDataWriter extends DBOutputTxtSerivce {

	public BOMgenggDBDataWriter(DataParserConfig dataParserConfig) {
	  	...
	  	try {
	        ...
	        for(int i=0;i<totalPage;i++){
	          ...
	          //XML文件AppReqBody之前数据
		    	...
		    	/*********************************************************/
		    	TableElement.addAttribute("name", "BOM更改");
	            ...
	            for (int j = 0; j < dataList.size(); j++) {
		        	...
		        	rowElement.addAttribute("MATNR", dataList.get(j).get("MATNR").toString().trim());
		        	rowElement.addAttribute("WERKS", dataList.get(j).get("WERKS").toString().trim());
		        	rowElement.addAttribute("POSNR", dataList.get(j).get("POSNR").toString().trim());
		        	rowElement.addAttribute("IDNRK", dataList.get(j).get("IDNRK").toString().trim());
		        	rowElement.addAttribute("AENNR", dataList.get(j).get("AENNR").toString().trim());
		        	rowElement.addAttribute("MENGE", dataList.get(j).get("MENGE").toString().trim());
		        	rowElement.addAttribute("MEINS", dataList.get(j).get("MEINS").toString().trim());
		        	rowElement.addAttribute("CZM", dataList.get(j).get("CAOZM").toString().trim());
				}
	            time ++ ;
	            /*********************************************************/
	            try {
