 ...
 *
 */
public class JtpcjhDbDateReader extends TxtInputDBSerivce{
	protected static Logger logger = Logger.getLogger(JtpcjhDbDateReader.class);	//定义日志方法
	...
	public JtpcjhDbDateReader(DataParserConfig dataParserConfig) {
	...
	public boolean beforeRecord(String line, String fileName, int lineNum){
		...
		if (lineNum==1||StringUtils.isEmpty(line)) {// 文件第一行不导入表
			...
			try{
				...
				//过滤掉类型为1的数据  lc 2016-09-23 0012894
				String leix = line.toString().substring(0, 1).trim();
				if ("1".equals(leix)) {
					result = false;
				}
			}catch(Exception e){
	...
	public void after() {
		...
		try{
			inList = dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).select("inPutddbh.queryOfhOfJtpc");
		}catch(RuntimeException e){
		...
		for(int i=0;i<inList.size();i++){
			...
			if(num==1){
				...
				if(shangXJSList.size()>0){
					...
					File_ErrorInfo(DbDataWriter.getUUID(),"3060",DbDataWriter.getUUID(),logInfo,logDate);
			...
			if(usercenter.contains(",")){
				...
				if(users!=null&&users.length>0){
					for(int i=0;i<users.length;i++){
						if(StringUtils.isEmpty(users[i])){
