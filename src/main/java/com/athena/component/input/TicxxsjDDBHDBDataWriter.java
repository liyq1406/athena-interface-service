 ...
 *
 */
public class TicxxsjDDBHDBDataWriter extends TxtInputDBSerivce {
	...
	public void before() {
		try{
			dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).execute("inPutddbh.ticxxsjTempDelete");
    ...
    public void after() {
		...
		.execute("inPutddbh.ticxxsjUpdate");
		//对主表与中间表进行合并
		dataParserConfig.getBaseDao().getSdcDataSource(dataParserConfig.getWriterConfig().getDatasourceId())
		.execute("inPutddbh.ticxxsjMerge");
		 //删除主表标记editor为temp的数据
		dataParserConfig.getBaseDao().getSdcDataSource(dataParserConfig.getWriterConfig().getDatasourceId())
		.execute("inPutddbh.ticxxsjDelete");
    }
