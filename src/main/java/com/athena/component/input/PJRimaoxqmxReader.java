 ...
 * @author hzg
 */
public class PJRimaoxqmxReader extends TxtInputDBSerivce{
	private String datasourceId = "";
	protected static Logger logger = Logger.getLogger(PJRimaoxqmxReader.class);	//定义日志方法
	public PJRimaoxqmxReader(DataParserConfig dataParserConfig) {
	...
	public void before() {
		try{ 
			...
			String inBanc = (String)dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).selectObject("inPutzbc.queryRiClvInMaoxqPJ",paramclv);
			if(inBanc!=null && !"".equals(inBanc)){
				paramclv.put("XUQBC", inBanc);
				List<Map<String, String>> inMaoxUc = dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).select("inPutzbc.queryRiClvInMaoxqUcPJ",paramclv);
				List<Map<String, String>> ckxUc = dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).select("inPutzbc.queryRiClvCkxUcPJ",paramclv);
				...
				if(inMaoxUc.size() != ckxUc.size()){
					...
					if(usercenter.length()>0){
						String lastBanc = (String)dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).selectObject("inPutzbc.queryNewClvMaoxqPJ",paramclv);
						if(lastBanc!=null && lastBanc.length()>0){
							paramclv.put("LASTBANC", lastBanc);
							paramclv.put("UC", usercenter.toString());
							dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).execute("inPutzbc.insertCLVLastMaoxqPJ",paramclv);
