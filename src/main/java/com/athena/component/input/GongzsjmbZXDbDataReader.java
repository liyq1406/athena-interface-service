 ...
 * @author hzg
 */
public class GongzsjmbZXDbDataReader extends TxtInputDBSerivce{
	protected static Logger logger = Logger.getLogger(GongzsjmbZXDbDataReader.class);	//定义日志方法 
	public GongzsjmbZXDbDataReader(DataParserConfig dataParserConfig) {
	...
	public void before() {
		try{
			...
			.execute("inPutzxc.gongzsjmbTDelete");
	...
	public void after() {
		try{
			...
			dataParserConfig.getBaseDao().getSdcDataSource(dataParserConfig.getWriterConfig().getDatasourceId())
			.execute("inPutzxc.updateGongzsjmbCreator");
			
			dataParserConfig.getBaseDao().getSdcDataSource(dataParserConfig.getWriterConfig().getDatasourceId())
			.execute("inPutzxc.gongzsjmbTMerge");
