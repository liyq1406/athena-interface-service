...
/**
 * 1410 IN0035内部要货令（准备层）,2340 内部要货令（执行层）
 * @author hzg
 *
 */
public class NBYHLDBDataWriter extends TxtWriterDBTask{

	public NBYHLDBDataWriter(DataParserConfig dataParserConfig, List<String> fieldList, List<String> updateFieldList, String fileName, List<Record> lineList) {
	...
	public boolean beforeRecord(Record record){
		try{
			if("04".equals(record.getString("YAOHLLB"))){
				record.put("BEIZ7","1");
			}
