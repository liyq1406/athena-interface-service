 ...
 * @author hzg
 */
public class GongzsjZXDbDataReader extends TxtInputDBSerivce{
	protected static Logger logger = Logger.getLogger(GongzsjZXDbDataReader.class);	//定义日志方法
	 private final static int PAGES = 5000; 
	public GongzsjZXDbDataReader(DataParserConfig dataParserConfig) {
	...
	public void before() {
		try{
			...
			.execute("inPutzxc.gongzsjmbTempDelete");
		}catch(RuntimeException e)
		{
			logger.error("线程--接口" + dataParserConfig.getId() +"清除ckx_gongzsjmb表时报错"+e.getMessage());
			throw new ServiceException("线程--接口" + dataParserConfig.getId() +"清除ckx_gongzsjmb表时报错"+e.getMessage());
		}
	}
	...
	public void after() {
		try{
			...
			dataParserConfig.getBaseDao().getSdcDataSource(dataParserConfig.getWriterConfig().getDatasourceId())
			.execute("inPutzxc.gongzsjmbMerge");
	...
	public void deleteOldDate() {
		try{
			...
			while(runnum>0){
				...
				.select("inPutzxc.querygongzsjmbOldDate");
	            ...
	            for(int i=0;i<pageNum;i++){
					...
					dataParserConfig.getBaseDao().getSdcDataSource(dataParserConfig.getWriterConfig().getDatasourceId())
					.executeBatch("inPutzxc.gongzsjmbMergeDelete",oneList);	
	            }
	            runnum--;
			}
			logger.info("接口" + interfaceId +"Merge表ckx_gongzsjmb结束");
