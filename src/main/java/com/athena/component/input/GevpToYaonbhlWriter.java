...
public class GevpToYaonbhlWriter extends TxtInputDBSerivce{

	private String datasourceId = "";
	public GevpToYaonbhlWriter(DataParserConfig dataParserConfig) {
		super(dataParserConfig);
		datasourceId = dataParserConfig.getWriterConfig().getDatasourceId();
	}
	
	/**
	 * 接口处理完成后更新CK_YAONBHL表的子仓库代码，将修改人改为2032
	 */
	@Override
	public void after() {
		try{
			Map<String,String> params = new HashMap<String,String>();
			params.put("INTEREDITOR", "2030temp");
			List<Map<String,String>> newyaohl = dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).select("inPutzxc.newyaohlList", params);
			if(newyaohl != null && newyaohl.size()>0){
				dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).executeBatch("inPutzxc.updateNbyaohlEditor",newyaohl);
				dataParserConfig.getBaseDao().getSdcDataSource(datasourceId).executeBatch("inPutzxc.updateGevpClzt",newyaohl);
			}
			//将本次接口的子仓库代码更新
			...
			logger.error("线程--接口" + interfaceId +"gevp更新CK_YAONBHL表ZICKBH子仓库编码时报错"+e.getMessage(),e);
			throw new ServiceException("线程--接口" + interfaceId +"gevp更新CK_YAONBHL表ZICKBH子仓库编码时报错"+e.getMessage());
		}
	}
